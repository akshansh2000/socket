...
// 'direct' [0,1] specifies the kind of algorithm: 1=direct, no augmented zero
// bits 'crcinit' is the initial CRC value belonging to that algorithm 'crcxor'
// is the final XOR value 'refin' [0,1] specifies if a data byte is reflected
// before processing (UART) or not 'refout' [0,1] specifies if the CRC will be
// reflected before XOR
...
void random_flip(char *rem_message) {
  for (int i = 0; i < 5; i++) {
    rem_message[i] = 'z' - rem_message[i];
  }
}
...
int connection_setup(int port, char *addr) {
  ...
  // create a new socket witch AF_INET for internet domain, stream socket
  // option, TCP(given by os) - reliable, connection oriented
  ...
  serv_addr.sin_port = htons(port); // convert a port number in host byte order
  ...
  setsockopt(socket_client, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv,
...
int server_communicate(int socket_client, char *formed_message) {
  ...
  if (strcmp(reply, "NACK") == 0) {
    fprintf(stderr,
            "Previous transmission had some error. Re-transmitting...\n");
    return 1;
...
int main(int argc, char **argv) {
  if (argc != 3) {
    fprintf(stderr, "Usage ./client ip_addr port errors");
    return -1;
  }
  float BER;
  printf("Enter if you want errors in transmission ? 0: No / 1: Yes ");
  scanf("%f", &BER);
  ...
  while (true) {
    ...
    do {
      ...
      unsigned long msg =
          crctablefast((unsigned char *)message, strlen(message));
      char *temp;
